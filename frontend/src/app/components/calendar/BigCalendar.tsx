...
export default function BigCalendar({ tasks }: CalendarProps) {
    ...
    const events = allTasks.map(task => ({
        ...
        dueDate: task.dueDate, // Add dueDate to match Task type
    }));
            ...
            <Tippy
                content={
                    <div className="p-2 text-left max-w-xs bg-gray-800 text-white rounded">
                        <div><strong>Assignee:</strong> {event.assignee}</div>
                        <div className="flex items-center gap-2 mb-1">
                            <strong>Column:</strong>
                            <span
                                className="px-2 py-0.5 rounded text-xs font-semibold"
                                style={{ background: columnColors[event.column] || '#888', color: '#1e293b' }}
                            >
                                {event.column}
                            </span>
                        </div>
                        ...
                        <div><strong>Due:</strong> {moment(event.start).format('DD MMM')} - {moment(event.end).format('DD MMM')}</div>
                        <div><strong>Tasks:</strong> <span dangerouslySetInnerHTML={{ __html: event.content }} /></div>
                    </div>
                }
                interactive={true}
                placement="top"
